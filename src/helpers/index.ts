...
import { createRetro,getRetro,addRetroAction,getRetroByHumanId } from "../msal/services";
import { Action, Retro } from "../types";

import React from "react";
import shortid from 'shortid';
import { BoardActionType, BOARD_STATE_MACHINE_VERSION } from "../statemachine/BoardStateMachine";

export const useRetro = () => {
    const [state, dispatch] = React.useContext(GlobalContext);

    return {
        create: async (retro: Partial<Omit<Retro, 'id'>>, retroTimeframe: string, retroGoal: string): Promise<Retro> => {
            const humanId = ('' + (Math.random() * 1000000000 + 1000000000)).substring(0, 6);

            const currentRetro = {
                name: 'Retro',
                ...(retro ? retro : {}),
                humanId,
                joinUrl: `${window.location.protocol}//${window.location.hostname}${window.location.port ? ':' + window.location.port : ''}/join/${humanId}`
            } as Retro;
            const id = await createRetro(currentRetro, state.user);
            const retrievedRetro = await getRetro(id);

            dispatch({ type: ActionType.SET_CURRENT_RETRO, payload: { retro: retrievedRetro } });

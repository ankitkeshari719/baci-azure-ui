...
type Props = {
  ...
  actionCreatedBy: number | undefined;
  users: any;
  developeActionWords: Word[];
  handleIsActionCloudOpen: () => void;
  isAllActionOpen: boolean;
  actionTopVotedCards: any;
  developeActionTopVotedCardsPrint_1: any;
  developeActionTopVotedCardsPrint_2: any;
};

export default function DevelopActionRedesign({
  actionColumnName,
  actionCreatedBy,
  users,
  developeActionWords,
  ...
  return (
    <Box>
      ...
      {/* Develope Action ? Section 2*/}
      <Row style={{ marginTop: '16px' }} id="view-top-voted-card-3">
        <Col
          xs="4"
          lg="4"
          className="d-flex justify-content-start align-items-center"
        >
          ...
          <Typography className="participantsResponded" ml={2}>
            {actionCreatedBy}/{users.length}
          </Typography>
        </Col>
        {developeActionWords.length === 0 ? (
          <Col
            xs={{ span: 3, offset: 5 }}
            lg={{ span: 4, offset: 4 }}
          ...
          >
            ...
            >
              {isAllActionOpen ? 'View Top Voted Cards' : 'View Word Cloud'}
            </Typography>
          </Col>
        )}
      </Row>
      {/* Develope Action ? Section print 2*/}
      <Row
        style={{ marginTop: '16px', display: 'none' }}
        id="view-top-voted-card-print-3"
      >
        <Col
          xs="12"
          className="d-flex justify-content-start align-items-center"
      ...
      {/* Develope Action ? Section 3*/}
      {actionTopVotedCards.length === 0 ? (
          ...
          >
            {isAllActionOpen ? (
              ...
              <Box sx={{ width: '100%' }}>
                ...
                {/*develop-action-Top-Voted-Cards */}
                <Row
                  style={{ marginTop: '16px' }}
                  id="develop-action-Top-Voted-Cards"
                >
                  {actionTopVotedCards.map((card: any, index: number) => {
                    return (
                      <Col
                ...
                {/* develop-action-Top-Voted-Cards-print */}
                <Box
                  id="develop-action-Top-Voted-Cards-print"
                  sx={{ display: 'none' }}
                >
                  <Row style={{ marginTop: '16px' }}>
                    {developeActionTopVotedCardsPrint_1.map(
                  ...
                  <Row style={{ marginTop: '16px' }}>
                    {developeActionTopVotedCardsPrint_2.map(
                      (card: any, index: number) => {
                        return (
                          <Col
                            xs="6"
